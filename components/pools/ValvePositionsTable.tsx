  ...
  const StatusBadge = ({ status, label }: { status?: string | null; label?: string }) => {
    ...
    return (
      <span
        ...
        className={`status-badge inline-flex items-center justify-center min-h-6 px-4 py-1 rounded-full text-base font-semibold border ${cls}`}
      >
        {status ?? ''}
      </span>
    );
              ...
              <div className="md:hidden mt-4 space-y-4">
                {tableRows.map((r, i) => (
                  <div
                    ...
                    data-label={r.label}
                    className={`bg-white border border-secondary-100 rounded-xl p-4 shadow-sm transition ring-2 ring-primary-500/0 ${highlightedId === `valve-${slug(r.label)}` ? 'ring-primary-500 bg-primary-50' : ''}`}
                    tabIndex={0}
                    onClick={() => {
                      const id = `valve-${slug(r.label)}`;
                      window.location.hash = id;
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="text-base font-bold text-secondary-900">{r.label}</div>
                      <span className="ml-2 rounded bg-secondary-100 text-secondary-900 font-semibold px-3 py-1 text-xs">
                        {r.selector}
                      </span>
                    </div>
                    <div className="divide-y divide-secondary-50">
                      <div className="flex justify-between items-center py-2">
                        <span className="font-medium text-secondary-600">{headers.sumidero}</span>
                        <StatusBadge status={r.sumidero} label={`${headers.sumidero} ${r.sumidero ?? ''}`} />
                      </div>
                      <div className="flex justify-between items-center py-2">
                        <span className="font-medium text-secondary-600">{headers.skimmers}</span>
                        <StatusBadge status={r.skimmers} label={`${headers.skimmers} ${r.skimmers ?? ''}`} />
                      </div>
                      <div className="flex justify-between items-center py-2">
                        <span className="font-medium text-secondary-600">{headers.limpiafondos}</span>
                        <StatusBadge status={r.limpiafondos} label={`${headers.limpiafondos} ${r.limpiafondos ?? ''}`} />
                      </div>
                      <div className="flex justify-between items-center py-2">
                        <span className="font-medium text-secondary-600">{headers.retorno}</span>
