import Link from "next/link";
import Image from "next/image";
import React from "react";

export default function Footer({ locale = "es" }: { locale?: string }) {
  const currentYear = new Date().getFullYear();
  
  // Helper function for translations
  const t = (es: string, en: string, ca: string) => {
    if (locale === "en") return en;
    if (locale === "ca") return ca;
    return es;
  };
  
  return (
    <footer className="bg-secondary-900 text-secondary-100 border-t border-secondary-800">
      <div className="container-custom section-padding">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {/* Brand Column */}
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="relative w-10 h-10 rounded-lg overflow-hidden">
                <Image 
                  src="/logo.jpeg" 
                  alt="Fontanería Logo" 
                  fill
                  className="object-cover"
                  unoptimized
                />
              </div>
              <div>
                <h3 className="font-bold text-white text-lg">Fontanería en Barcelona</h3>
                <p className="text-sm text-secondary-400">
                  {t('Urgencias 24h', '24/7 Emergency', 'Urgències 24h')}
                </p>
              </div>
            </div>
            <p className="text-sm text-secondary-300 leading-relaxed">
              {t('Servicios profesionales de fontanería y mantenimiento de piscinas en Terrassa, Vallès y comarca de Barcelona. Emergencias 24 horas.',
                 'Professional plumbing and pool maintenance services in Terrassa, Vallès and Barcelona region. 24-hour emergencies.',
                 'Serveis professionals de fontaneria i manteniment de piscines a Terrassa, Vallès i comarca de Barcelona. Urgències 24 hores.')}
            </p>
          </div>

          {/* Services Column */}
          <div>
            <h4 className="font-semibold text-white mb-4">
              {t('Servicios', 'Services', 'Serveis')}
            </h4>
            <ul className="space-y-2 text-sm">
              <li>
                <Link 
                  href={`/${locale}/servicios/fontaneria`} 
                  className="text-secondary-300 hover:text-primary-400 transition-colors inline-flex items-center group"
                >
                  <span className="mr-2 text-primary-400">→</span>
                  {t('Fontanería', 'Plumbing Services', 'Fontaneria')}
                </Link>
              </li>
              <li>
                <Link 
                  href={`/${locale}/servicios/piscinas`} 
                  className="text-secondary-300 hover:text-primary-400 transition-colors inline-flex items-center group"
                >
                  <span className="mr-2 text-primary-400">→</span>
                  {t('Mantenimiento de Piscinas', 'Pool Maintenance', 'Manteniment de Piscines')}
                </Link>
              </li>
              <li>
                <Link 
                  href={`/${locale}/contacto`} 
                  className="text-secondary-300 hover:text-primary-400 transition-colors inline-flex items-center group"
                >
                  <span className="mr-2 text-primary-400">→</span>
                  {t('Contacto y Presupuesto', 'Contact & Quote', 'Contacte i Pressupost')}
                </Link>
              </li>
            </ul>
          </div>

          {/* Contact Column */}
          <div>
            <h4 className="font-semibold text-white mb-4">
              {t('Contacto', 'Contact', 'Contacte')}
            </h4>
            <ul className="space-y-3 text-sm">
              <li className="flex items-start gap-2">
                <svg className="w-5 h-5 text-primary-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                <a href="tel:+34677133242" className="text-secondary-300 hover:text-primary-400 transition-colors">
                  +34 677 133 242
                </a>
              </li>
              <li className="flex items-start gap-2">
                <svg className="w-5 h-5 text-primary-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <a 
                  href="mailto:fontanerialowcost24@gmail.com" 
                  className="text-secondary-300 hover:text-primary-400 transition-colors break-all"
                >
                  fontanerialowcost24@gmail.com
                </a>
              </li>
              <li className="flex items-start gap-2">
                <svg className="w-5 h-5 text-primary-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <span className="text-secondary-300">
                  {t('Provincia de Barcelona', 'Barcelona Province', 'Província de Barcelona')}
                </span>
              </li>
            </ul>
          </div>

          {/* Schedule & CTA Column */}
          <div>
            <h4 className="font-semibold text-white mb-4">
              {t('Horario', 'Business Hours', 'Horari')}
            </h4>
            <div className="space-y-3 text-sm">
              <div className="bg-accent-500/10 border border-accent-500/20 rounded-lg p-3">
                <p className="text-accent-400 font-semibold flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  {t('Abierto 24/7', 'Open 24/7', 'Obert 24/7')}
                </p>
                <p className="text-secondary-300 mt-1">
                  {t('Servicio de urgencias disponible', 'Emergency service available', 'Servei d\'urgències disponible')}
                </p>
              </div>
              
              <a 
                href="https://wa.me/34677133242" 
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-3 rounded-lg transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
                WhatsApp
              </a>
            </div>
          </div>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="border-t border-secondary-800">
        <div className="container-custom py-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-secondary-400">
            <p>
              © {currentYear} Fontanería en Barcelona. {t('Todos los derechos reservados', 'All rights reserved', 'Tots els drets reservats')}.
            </p>
            <div className="flex items-center gap-4">
              <Link href={`/${locale}`} className="hover:text-primary-400 transition-colors">
                {t('Inicio', 'Home', 'Inici')}
              </Link>
              <span className="text-secondary-700">•</span>
              <Link href={`/${locale}/contacto`} className="hover:text-primary-400 transition-colors">
                {t('Contacto', 'Contact', 'Contacte')}
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
